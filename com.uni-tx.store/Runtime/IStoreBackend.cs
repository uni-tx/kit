...
{
    /// <summary>
    /// Where store progress is stored.
    /// </summary>
    /// <remarks>
    /// <para>
    /// The seam that lets a game ship offline-first and add server authority later without
    /// touching a single call site. <see cref="LocalStoreBackend"/> keeps everything on the
    ...
    /// whether a claim really happened.
    /// </para>
    /// </remarks>
    public interface IStoreBackend
    {
        ...
        UniTask<StoreSavedData> LoadAsync(string saveId, CancellationToken cToken = default);
        ...
        /// </remarks>
        UniTask SaveAsync(StoreSavedData data, bool immediate, CancellationToken cToken = default);
    }
}
