...
{
    /// <summary>
    /// One reward a quest grants on claim.
    /// </summary>
    /// <remarks>
    /// The fields mirror the generic <see cref="RewardData"/> so the default granter can map
    /// the quest onto the kit's reward service without a translation table.
    /// </remarks>
    [Serializable]
    public sealed class QuestRewardData
    {
        [Tooltip("Unique reward id within the quest. Part of the idempotent grant id, so " +
                 "changing it on a live quest re-delivers the reward once.")]
        ...
        /// <summary>
        /// Gets the reward id, unique within the quest.
        /// </summary>
        ...
        /// </summary>
        public bool IsValid => !string.IsNullOrWhiteSpace(_rewardId) && !string.IsNullOrWhiteSpace(_itemId) && _amount > 0;
    }
}
