...
{
    /// <summary>
    /// The default <see cref="IQuestRewardGranter"/>: maps each reward onto the generic
    /// <see cref="RewardData"/> and delivers it through <see cref="IRewardService"/>.
    /// </summary>
    /// <remarks>
    /// <para>
    /// When the kit's rewards package is installed and its service is registered, quest
    ...
    /// <see cref="IQuestRewardGranter"/>.
    /// </para>
    /// <para>
    /// The quest's period-boundary grant id is passed through, so even a replay that slips
    /// past the quest system's own ledger cannot double-deliver into the economy.
    /// </para>
    /// </remarks>
    public sealed class QuestRewardGranter : IQuestRewardGranter
    {
        ...
        public QuestRewardGranter(IRewardService rewards)
        {
            _rewards = rewards ?? throw new ArgumentNullException(nameof(rewards));
        }

        /// <inheritdoc />
        public async UniTask<bool> GrantAsync(QuestData quest, QuestRewardData reward,
