...
Thin UniTx-flavoured facade over Unity's Localization package: async locale switching and key lookup.

**Unity 6.5 (6000.5) or newer** · MIT · v1.3.0

A thin facade over Unity's Localization package: UniTask-shaped async,
locale switching that waits for tables to reload, and lookups that degrade to the key
instead of throwing.
...
"dependencies": {
  ...
  "com.uni-tx.localization": "https://github.com/uni-tx/kit.git?path=/com.uni-tx.localization#localization@1.3.0"
}
```
...
4. `https://github.com/uni-tx/kit.git?path=/com.uni-tx.localization#localization@1.3.0`
...
```csharp
await UniLocalization.InitializeAsync(token);
await UniLocalization.SetDeviceLocaleAsync(token);

var title = await UniLocalization.GetStringAsync("UI", "menu_title", token);
```

## Samples

Import from **Package Manager ▸ UniTx Localization ▸ Samples**.

- **Localized Text** — Switching locale at runtime and binding a localized string table entry to a label.

## Notes

- The assembly is guarded by a version define, so a project without
  `com.unity.localization` omits it instead of failing to compile.
- Lookups return the key when an entry is missing, so gaps show up as visible placeholder
