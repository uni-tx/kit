...
{
    /// <summary>
    /// Where daily reward progress is stored.
    /// </summary>
    /// <remarks>
    /// <para>
    /// The seam that lets a game ship offline-first and add server authority later without
    /// touching a single call site. <see cref="LocalDailyRewardsBackend"/> keeps everything
    /// on the device; a remote implementation forwards the same operations to a validated
    ...
    /// whether a claimed day was really claimed.
    /// </para>
    /// </remarks>
    public interface IDailyRewardsBackend
    {
        /// <summary>
        ...
        UniTask<DailyRewardsSavedData> LoadAsync(string saveId, CancellationToken cToken = default);
        ...
        /// </remarks>
        UniTask SaveAsync(DailyRewardsSavedData data, bool immediate,
