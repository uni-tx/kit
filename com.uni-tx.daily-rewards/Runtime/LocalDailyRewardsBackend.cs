...
{
    /// <summary>
    /// Stores daily reward progress on the device through the kit's serialisation service.
    /// </summary>
    /// <remarks>
    /// The default backend, and a complete one: a single-player game needs nothing else.
    /// Writes go through <see cref="ISerialisationService"/>, so they inherit its batching
    /// and its atomic temp-file-then-replace behaviour — an interrupted write costs the last
    /// batch rather than the whole calendar.
    /// </remarks>
    public sealed class LocalDailyRewardsBackend : IDailyRewardsBackend, IInjectable
    {
        ...
        /// </summary>
        public LocalDailyRewardsBackend()
        {
        }
        ...
        public LocalDailyRewardsBackend(ISerialisationService serialisation) =>
        ...
        public void Inject(IResolver resolver) =>
            _serialisation ??= resolver.Resolve<ISerialisationService>();
            ...
            CancellationToken cToken = default)
        {
            ...
            // Flush turns a queued write into a landed one. Worth the disk hit at a
            // checkpoint: a claim lost to a crash is a support ticket, not a rounding error.
